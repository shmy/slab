...
1. **记录内容**：所有产生变更的写操作；创建（before 空）与删除（after 空）都记；操作人取 `OperatorContext` 必记；同时记录业务动作（`action`，如 `account.create` / `purchase_order.approve`）、客户端 IP 与 User-Agent。不记录请求级审计（监控已覆盖），不记录失败请求（无变更即无历史）。
...
3. **写入**：业务切片在自己的写事务内同步调用 `audit_contract::record`（传 `txn.as_mut()`），与业务写同事务原子——回滚即消失、提交即可见。无 Outbox、无消费者、无异步延迟、无需幂等。**禁止"提交后再写"**（事务成功但审计丢失的黑洞窗口）。
...
6. **跨域写 Port**：`audit_contract::record` 是 contract 内带 SQL 实现的**跨域同事务写**入口。本仓 Port 先例（`{Domain}Port` 默认方法自带读 SQL）只覆盖读，audit 是第一个跨域写——以动词命名（`record` vs 只读 Port 的名词方法 `by_id`）并写文档注释区分，不违反"contract 间互不依赖 / 不 import features/{other} runtime crate"规则。
7. **操作人上下文**：`http_auth::extract::operator_context::OperatorContext`（操作人 + IP + UA）合并提取器。放 `http_auth` 而非 `audit_contract`（contract 不得依赖 infrastructure，否则把鉴权栈拖进所有消费方），也非 `web`（`http_auth → web` 已成，反向成环）。按内容而非消费方命名：登录历史、安全日志同样需要这份上下文。
...
9. **保留**：永久保留，不设 TTL；手动清理 / 归档脚本按需重建（沿用第一版的 cleanup 思路）。
...
- 写端点需接入变更历史（同事务调 `audit_contract::record`）；**漏接不产生编译错误**，靠代码评审 + 端点测试断言 `audit_logs` 兜底（identity 的账户创建 / 更新 / 删除已接入作示范）。
