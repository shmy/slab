...
mod tests {
    ...
    async fn test_approve_success(pool: sqlx::PgPool) {
        ...
        let from_wh = tests::insert_test_warehouse(&state.pg_pool, "TRF-A-A").await;
        let to_wh = tests::insert_test_warehouse(&state.pg_pool, "TRF-A-B").await;
        let id = ID::new();
        sqlx::query!(
        ...
        // 变更历史：update 类型，状态 Submit(1) → Approved(3)，before 无 approved_at
        ...
        assert!(before["approved_at"].is_null());
        assert!(after["approved_at"].as_str().is_some());
    }
}
