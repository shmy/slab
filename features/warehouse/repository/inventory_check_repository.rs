...
pub struct InventoryCheckRepository;

/// 审批动作返回的锁定快照：供调用方在同一事务内执行库存台账副作用。
#[derive(Debug)]
pub(crate) struct LockedInventoryCheck {
    pub warehouse_id: i64,
}

impl InventoryCheckRepository {
    /// 提交：锁定读 + 状态机校验 + 写入新状态。
    pub async fn submit(conn: &mut PgConnection, id: &ID) -> Result<()> {
        let status = Self::lock_status(conn, id).await?;
        let new_status = CH_FLOW.submit_status(status)?;
        sqlx::query!(
            r#"UPDATE inventory_checks SET status = $1 WHERE id = $2"#,
            new_status,
            id as _
        )
        .execute(conn)
    ...
    pub async fn approve(conn: &mut PgConnection, id: &ID) -> Result<LockedInventoryCheck> {
        let row = sqlx::query!(
            r#"SELECT status, warehouse_id
        ...
        let new_status = CH_FLOW.approve_status(row.status)?;
        sqlx::query!(
            r#"UPDATE inventory_checks SET status = $1, approved_at = NOW() WHERE id = $2"#,
            new_status,
            id as _
        )
        .execute(&mut *conn)
        .await?;
        Ok(LockedInventoryCheck {
    ...
    async fn lock_status(conn: &mut PgConnection, id: &ID) -> Result<i16> {
        let row = sqlx::query!(
            r#"SELECT status FROM inventory_checks WHERE id = $1 FOR UPDATE"#,
            id as _
        )
        .fetch_optional(&mut *conn)
        .await?
...
use warehouse_contract::value_object::InventoryCheckStatus;
    ...
    async fn test_submit_draft_success(pool: sqlx::PgPool) {
        ...
        InventoryCheckRepository::submit(&mut txn, &id)
            .await
            .unwrap();
        txn.commit().await.unwrap();

        let status = sqlx::query_scalar!("SELECT status FROM inventory_checks WHERE id = $1", &*id)
    ...
    async fn test_submit_already_submitted_rejected(pool: sqlx::PgPool) {
        ...
        let mut conn = pool.acquire().await.unwrap();
        let mut txn = conn.begin().await.unwrap();
        let err = InventoryCheckRepository::submit(&mut txn, &id)
            .await
            .unwrap_err();
    ...
    async fn test_approve_pending_success(pool: sqlx::PgPool) {
        ...
        let mut conn = pool.acquire().await.unwrap();
        let mut txn = conn.begin().await.unwrap();
        let locked = InventoryCheckRepository::approve(&mut txn, &id)
            .await
            .unwrap();
        txn.commit().await.unwrap();

        let row = sqlx::query!(
            "SELECT status, approved_at FROM inventory_checks WHERE id = $1",
            &*id
        )
        .fetch_one(&mut *pool.acquire().await.unwrap())
        .await
    ...
    async fn test_approve_draft_rejected(pool: sqlx::PgPool) {
        ...
        let mut conn = pool.acquire().await.unwrap();
        let mut txn = conn.begin().await.unwrap();
        let err = InventoryCheckRepository::approve(&mut txn, &id)
            .await
            .unwrap_err();
        assert!(err.to_string().contains("invalid_status_transition"));
    }
    ...
    async fn test_not_found(pool: sqlx::PgPool) {
        ...
        let mut conn = pool.acquire().await.unwrap();
        let mut txn = conn.begin().await.unwrap();
        let err = InventoryCheckRepository::submit(&mut txn, &ID::new())
            .await
            .unwrap_err();
